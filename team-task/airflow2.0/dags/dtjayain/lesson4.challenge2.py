...
branch_odd_task = BashOperator(
    task_id="branch_odd",
    bash_command=f'echo "The date is odd number"',
    dag=dag,
)

def _odd_process():
    logging.info("This is just odd process after branch_odd_task")

def curr_date(execution_date,**kwargs):
    logging.info(f"Execution date: {execution_date}")

def final_f():
    logging.info("DAG was run successfully up until this point")
...
branch_even_task = BashOperator(
    task_id="branch_even",
    bash_command=f'echo "echo "The date is even number"',
    dag=dag,
)
...
[branch_even_task, odd_process_task] >> curr_date_task >> final_task
is_latest_task >> dummy_notif_task