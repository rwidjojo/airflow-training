...
def generate_sql(prev_ds, ds,x):
    ...
    WHERE order_date > '{prev_ds}' and order_date <= '{ds}'
    GROUP BY order_date, ship_country
    ORDER BY order_date
    """

def migrate_odd_day(prev_ds, ds, **kwargs):

    # declare sql string
    sql = generate_sql(prev_ds, ds,True)
...
def migrate_even_day(prev_ds, ds, **kwargs):
...
def _check_row_task(ds, prev_ds, **kwargs):
    read_sql = f"""
    ...
    for row in result:
        print(f'Record inserted {prev_ds} - {ds} is {row} rows')
