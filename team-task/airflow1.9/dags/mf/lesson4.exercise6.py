...
def migrate_infive_day():
    log.info('Migrate in multiply of five')
...
migrate_infive_task = PythonOperator(
    task_id="migrate_infive",
    python_callable=migrate_infive_day,
    provide_context=True,
    dag=dag,
)
...
mult_branch = BranchPythonOperator(
  task_id='migration_branch',
  provide_context=True,
  python_callable=_pick_migration,
  dag=dag
