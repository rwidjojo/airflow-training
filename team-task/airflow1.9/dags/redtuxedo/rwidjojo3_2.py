...
dag = DAG(
    f'{owner["name"]}.lesson3.excercise2',
...
def migrate_data():
  src_conn = PostgresHook(postgres_conn_id='rwidjojo_postgres')
  dest_conn = PostgresHook(postgres_conn_id='rwidjojo_postgres_2')
  result = src_conn.get_records('select * from public.orders limit 10')
  insert_sql = f"""
    INSERT INTO public.{owner["name"]}_orders(order_id,customer_id,employee_id,order_date,required_date,shipped_date,ship_via,freight,ship_name,ship_address,ship_city,ship_region,ship_postal_code,ship_country)
