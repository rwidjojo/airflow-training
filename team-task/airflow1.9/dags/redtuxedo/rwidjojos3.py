...
dag = DAG(f"{owner}.lesson4.s3",
default_args=default_args,
schedule_interval=None
)  # "schedule_interval=None" means this dag will only be run by external commands

TEST_BUCKET = 'pmi-airflow'
TEST_KEY = 'pmi.txt'
LOCAL_FILE = '/home/ubuntu/airflow/dags/redtuxedo/pmi.txt'
...
def download_file(bucket,destination):
    ...
    try:
        s3.Bucket(TEST_BUCKET).download_file(TEST_KEY, '/home/ubuntu/airflow/dags/redtuxedo/my_local_image.jpg')
    except :
...
def upload_file(source, bucket):
    ...
    s3.meta.client.upload_file(LOCAL_FILE, bucket, 'Widjojo_Ronald/hello.txt')
