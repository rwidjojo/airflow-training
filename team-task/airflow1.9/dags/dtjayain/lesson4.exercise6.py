# Instruction 
# - add new branch if day is multiplication of 5 for example 5, 10, 15 etc
# - in new branch count result is multiplicate by 5
...
def generate_sql(ds, multiplier):
    return f"""
...
migrate_3_task = PythonOperator(
    task_id="migrate_3_task",
    python_callable=migrate_3_day,
    provide_context=True,
    dag=dag,
)
...
def _pick_migration(**kwargs):
  ...
  if execution_date.day % 3 == 0:
    return 'migrate_3_task' # returning task id `migrate_3_task`
...
greet_task >> migration_branch >> [migrate_even_task, migrate_odd_task, migrate_3_task]